    ...
    <section className="border-b border-white/10 bg-[#0d1011]">
      <div className="container-page grid divide-y divide-white/10 md:grid-cols-3 md:divide-x md:divide-y-0">
        {pathways.map(([index, title, description]) => (
          <article className="px-1 py-9 md:px-7" key={title}>
            <span className="text-[0.65rem] font-bold tracking-[0.16em] text-[var(--gold)]">{index}</span>
            <h2 className="mt-3 text-2xl font-semibold tracking-tight text-[var(--ink)]">{title}</h2>
