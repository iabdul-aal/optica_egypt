...
import { ArrowUpRight, GraduationCap, Info, MapPin, Sparkles, UserCheck } from "lucide-react"
import type { Chapter } from "@/lib/chapters"

// Authentic geographic vector boundary of Egypt (Natural Earth)
// ViewBox: 0 100 2000 1800 (W: 2000, H: 1800)
...
export function ChaptersMap({ chartered }: ChaptersMapProps) {
  ...
  // AUC Campus coordinates in the Natural Earth 2000x1800 coordinate frame:
      ...
      <div className="relative p-4 sm:p-8 bg-[var(--surface)] border-b lg:border-b-0 lg:border-r border-[var(--line)] flex flex-col justify-between">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--line-subtle)] pb-4">
          ...
          <span className="text-[10px] font-mono px-2 py-0.5 rounded-sm border border-[var(--line)] bg-[var(--surface-raised)] text-[var(--ink-faint)]">
            Full Geographic Vector Map
          </span>
        </div>

        {/* Scaled Authentic Map of Egypt */}
        <div className="relative my-4 w-full flex items-center justify-center overflow-hidden">
          <svg
            viewBox="0 100 2000 1800"
            className="w-full h-auto max-h-[520px] select-none"
            fill="none"
            stroke="currentColor"
            aria-label="Geographic Map of Egypt showing student chapters and university hubs"
          >
            <defs>
              {/* Subtle grid pattern for cartographic feel */}
            ...
            {/* Background Grid */}
            <rect x="0" y="100" width="2000" height="1800" fill="url(#egypt-grid)" />

            {/* Egypt National Boundary and Territory */}
            <path
            ...
            {/* River Nile Annotation */}
            <g className="font-mono text-[16px] tracking-widest font-semibold fill-[var(--gold)] opacity-70 select-none">
              <text x="1030" y="980">River Nile</text>
            </g>

            {/* Major Geographic Anchors */}
            {/* Alexandria Anchor */}
            <circle cx="759" cy="282" r="4" fill="var(--ink-faint)" opacity="0.6" />
            <text x="690" y="270" fill="var(--ink-faint)" fontSize="15" fontFamily="monospace" opacity="0.7">
              Alexandria
            </text>

            {/* Cairo Anchor */}
            <circle cx="1095" cy="505" r="4" fill="var(--ink-faint)" opacity="0.6" />
            <text x="1030" y="525" fill="var(--ink-faint)" fontSize="15" fontFamily="monospace" opacity="0.7">
              Cairo
            </text>

            {/* ── Interactive AUC Chapter Pin Marker ── */}
            <g
              role="button"
              tabIndex={0}
              onClick={() => setSelectedChapter(aucChapter)}
              onKeyDown={(e) => {
            ...
            >
              ...
              <circle
                ...
                strokeWidth="3.5"
              />
              <circle cx={aucPinX} cy={aucPinY} r="5" fill="var(--gold)" />
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-[var(--line-subtle)] text-xs font-mono">
          ...
          <div className="flex items-center gap-2 text-[var(--ink-faint)]">
            ...
            <span>Chartering Open Nationwide to All Universities</span>
          </div>
        </div>
      </div>
