          ...
          <div>
            {news.map((item, index) => (
              <article className="grid grid-cols-[2.4rem_1fr] gap-x-3 gap-y-3 border-b border-white/10 py-7 md:grid-cols-[3.4rem_10.5rem_1fr] md:gap-x-6" key={item.id}>
                ...
                <p className="pt-0.5 text-[0.64rem] font-bold leading-5 tracking-[0.11em] text-[var(--ink-soft)] md:pt-1">{item.category.toUpperCase()} <span className="text-[var(--gold)]">/</span> {item.date}</p>
