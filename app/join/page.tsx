...
export default function JoinPage() {
  return <>
    ...
    <section className="section-space"><div className="container-page grid gap-12 lg:grid-cols-[.82fr_1.18fr]"><div><p className="eyebrow">Why join</p><h2 className="section-title mt-3">Come as you are. Grow from there.</h2><ul className="mt-8 grid gap-0 border-y border-white/15">{reasons.map((reason, index) => <li key={reason} className="flex gap-4 border-b border-white/10 py-4 text-sm leading-6 text-[var(--ink-soft)] last:border-b-0"><span className="font-mono text-[var(--gold)]">0{index + 1}</span>{reason}</li>)}</ul><div className="mt-9 flex flex-wrap gap-3"><a href={siteConfig.joinFormUrl} target="_blank" rel="noreferrer" className="btn-primary">Open membership form <ExternalLink size={15} /></a><a href={`mailto:${siteConfig.email}`} className="btn-secondary"><Mail size={15} /> Email us</a></div></div><ContactForm /></div></section>
  </>
}
